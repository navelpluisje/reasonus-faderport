...
local function set_visibility(tracks)
...
local function get_tracks_to_show()
  local matches = {}
  ...
  for i = 1, reaper.CountTracks(0) do
    ...
    local tr = reaper.GetTrack(0, i - 1)
    local hasInstrument = reaper.TrackFX_GetInstrument(tr)
    local idx = math.floor(reaper.GetMediaTrackInfo_Value(tr, "IP_TRACKNUMBER"))

    if (hasInstrument > -1) then
      matches[idx] = true
    end
  end
