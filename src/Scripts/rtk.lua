...
rtk = (function()
  __mod_rtk_core = (function()
    ...
    function rtk.new_theme(name, base, overrides) assert(not base or rtk.themes[base],
        string.format('base theme %s not found', base))
      assert(not rtk.themes[name],
        string.format('theme %s already exists', name))
      local theme = base and table.shallow_copy(rtk.themes[base])
          or {}
  ...
  __mod_rtk_type = (function()
    ...
    __mod_rtk_middleclass = (function()
      ...
      function middleclass.class(name, super) assert(type(name) == 'string',
          "A name (string) is needed for the new class")
  ...
  __mod_rtk_animate = (function()
    ...
    rtk.easing = { ['linear'] = function(x) return x
      ...
      ['in-quint'] = function(x) return x * x * x * x * x
            ...
            2)
      end, ['in-expo'] = function(x) return (x == 0) and 0 or 2 ^ (10 * x - 10) end,
    ...
    local _table_stepfuncs = { [1] = function(widget, anim) local x = anim.easingfunc(anim.pct)
      return { _resolve(x,
        anim.src[1], anim.dst[1]) }
    end,
  ...
  __mod_rtk_colour = (function()
    ...
    function rtk.colour.mod(colour, hmul, smul, vmul, amul) local h, s, v, a = rtk.colour.hsv(colour)
      return rtk.colour
    ...
    function rtk.colour.get_icon_style(colour, under) return rtk.colour.luma(colour, under) >
          rtk.light_luma_threshold and 'dark' or 'light'
    end
  ...
  __mod_rtk_image = (function()
    ...
    function rtk.Image:popdest() assert(gfx.dest == self.id,
        'rtk.Image.popdest() called on image that is not the current drawing target')
      rtk.popdest(self.id)
    end
    ...
    function rtk.Image:recolour(colour) local r, g, b, _ = rtk.colour.rgba(colour)
      return self:filter(0, 0, 0, 1.0, r
        , g, b, 0)
    end

    function rtk.Image:filter(mr, mg, mb, ma, ar, ag, ab, aa) self:pushdest()
      gfx.muladdrect(self.x, self.y, self.w,
        self.h, mr, mg, mb, ma, ar, ag, ab, aa)
      self:popdest()
      return self
    end

    function rtk.Image:rect(colour, x, y, w, h, fill) self:pushdest()
      rtk.colour.set(colour)
      gfx.rect(x, y, w, h,
        fill)
      self:popdest()
  ...
  __mod_rtk_widget = (function()
    ...
    rtk.Widget.register { x = rtk.Attribute { default = 0, reflow = rtk.Widget.REFLOW_FULL },
      ...
      w = rtk.Attribute { type = 'number', reflow = rtk.Widget.REFLOW_FULL,
        animate = function(self, anim) local val = anim.resolve(anim.easingfunc(anim.pct))
          return (
      ...
      bg = rtk.Attribute { calculate = function(self, attr, value, target) return value and
            { rtk.colour.rgba(value) }
      end, }, scroll_on_drag = true, show_scrollbar_on_drag = true,
  ...
  __mod_rtk_popup = (function()
    ...
    rtk.Popup.register { anchor = rtk.Attribute { reflow = rtk.Widget.REFLOW_FULL },
      ...
      bg = rtk.Attribute { default = function(self, attr) return rtk.theme.popup_bg or
            { rtk.colour.mod(rtk.theme.bg, 1, 1, rtk.theme.popup_bg_brightness, 0.96) }
      end, },
    ...
    function rtk.Popup:close() if not self.calc.visible or self.closed then
      ...
      self.closed = true
      self:animate { attr = 'alpha', dst = 0, duration = 0.15 }:done(function() self:hide()
        self:attr('alpha', 1)
        self
            .window:remove(self)
      end)
  ...
  __mod_rtk_container = (function()
    ...
    function rtk.Container:reorder_before(widget, target) local targetidx = self:get_child_index(target)
      return self
          :reorder(widget, targetidx)
    end

    function rtk.Container:reorder_after(widget, target) local targetidx = self:get_child_index(target)
      return self:
          reorder(widget, targetidx + 1)
    end
  ...
  __mod_rtk_window = (function()
      ...
      if attr == 'bg' then
        ...
        if rtk.has_js_reascript_api then
          if self._gdi_brush then
            ...
            reaper.atexit(function() reaper.JS_GDI_DeleteObject(self._gdi_brush)
              reaper.JS_GDI_DeleteObject(self
                ._gdi_pen)
            end)
          end
    ...
    local function verify_hwnd_coords(hwnd, x, y) local _, hx, hy, _, _ = reaper.JS_Window_GetClientRect(hwnd)
      return hx
          == x and hy == y
    end
  ...
  __mod_rtk_box = (function()
    ...
    function rtk.Box:initialize(attrs, ...) rtk.Container.initialize(self, attrs, self.class.attributes.defaults, ...)
      assert(self
        .orientation, 'rtk.Box cannot be instantiated directly, use rtk.HBox or rtk.VBox instead')
    end
  ...
  __mod_rtk_button = (function()
    ...
    rtk.Button.register { [1] = rtk.Attribute { alias = 'label' },
  ...
  __mod_rtk_entry = (function()
    ...
    local function is_word_break_character(value, pos) local c = value:sub(pos, pos)
      return c ~= '_' and
          c:match('[%c%p%s]')
    end
  ...
  __mod_rtk_slider = (function()
    ...
    function rtk.Slider:_handle_mouseup(event) local ok = rtk.Widget._handle_mouseup(self, event)
      self:
          _animate_thumb_overlays(event, nil, true)
    ...
    function rtk.Slider:_handle_focus(event, context) self:_animate_thumb_overlays(event, true)
      return rtk.Widget._handle_focus(self
        , event, context)
    end
